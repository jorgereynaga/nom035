...
class Index(LoginRequiredMixin,View):
	...
	def get(self, request, *args, **kwargs):
		...
		ctx['candidates']=[{'id':c.id,'nombre':c.nombre,'email':c.email,'puesto':c.puesto,'tipo':c.tipo,'completed':c.test_sessions.filter(status='completada').exists()} for c in Candidate.objects.filter(user=request.user).order_by('-record_create')[:3]]
