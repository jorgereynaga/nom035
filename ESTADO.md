...
- **Pendiente aún:** el sidebar de index.html y de los templates psico_*.html sigue con el link "Evaluaciones" apuntando a `{% url 'candidatos' %}` (duplicado con "Candidatos") — falta cambiarlo a `{% url 'instrumentos_catalogo' %}` en TODOS los templates que tengan ese link (cada uno trae su sidebar hardcodeado, no hay {% include %})
...
### Sidebar — link "Evaluaciones" duplicado (ver arriba en Catálogo)
Cambiar en index.html y todos los psico_*.html: `{% url 'candidatos' %}` → `{% url 'instrumentos_catalogo' %}` donde corresponda al link "Evaluaciones"
