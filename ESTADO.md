...
## Sesion 2026-08-05/06 -- gating de plan, limites anti-abuso, precios, seguridad basica, rediseno de evaluaciones y dashboard, SEO -- TODO DESPLEGADO A main, PENDIENTE CONFIRMAR ULTIMO DEPLOY EN VPS

Los 2 pendientes de la entrada anterior (texto Paso 4 + boton "¿Como empiezo...?") se verificaron en navegador y se cerraron al abrir esta sesion (commit `2985560c`). A partir de ahi, sesion larga con multiples features, cada una en su propia rama -> merge a `auditoria-local` -> merge a `main` -> push. Ruta oficial del repo: `C:\NormaIA-Pruebas\nom035` (VPS: `/webapps/NormaIA`, deploy = `cd /webapps/NormaIA && git pull && docker compose up -d --build web`, NUNCA correr migrate aparte).

### 1. Quitar gates de plan en alta de centros/empleados/candidatos -- DESPLEGADO
Peticion de los socios: no limitar creacion de Centros de Trabajo ni alta de empleados/candidatos psicometricos por falta de plan -- solo bloquear el CONSUMO real (envio de cuestionarios NOM-035, asignacion de instrumento psicometrico). Se investigo el codigo real antes de tocar nada: los gates estaban en el lugar equivocado (`WorkplaceFormView.get()`, `EmployeeFormView.get()`, `CandidateCreateView.post()` bloqueaban ANTES de crear/dar de alta), mientras que el consumo real (`SaveAnswers` para encuestas via `nom035_creditos`, `AssignTestView` para instrumentos via `psico_evaluaciones_disponibles`/`psico_demo`) ya bloqueaba correctamente y no se toco. Se quitaron los 3 gates de alta (6 lineas). Commit `ea48c84c`.

### 2. Limites anti-abuso (de cara a pruebas de carga con ingenieros externos) -- DESPLEGADO
Al quitar los gates de plan, quedo sin ningun tope la creacion de centros/empleados/candidatos. Jorge confirmo 3 limites de autoservicio: `employee_num` <= 3000 por centro, <= 200 centros de trabajo por cuenta, <= 2000 candidatos psicometricos por cuenta -- con mensaje invitando a contactar para cuenta empresarial en vez de error generico. Verificado creando realmente 200 centros + candidato 2000 vs 2001 contra la app real. Commit `cfeee520`.
PENDIENTE FUTURO registrado en memoria: revisar que ningun copy de marketing diga "ilimitado" (NOM-035/psicometricos) de forma que contradiga estos topes -- riesgo de publicidad enganosa.

### 2b. Auditoria de seguridad basica antes de las pruebas de carga -- HALLAZGOS Y FIXES
Jorge preguntara si la plataforma esta lista para que ingenieros amigos hagan pruebas "tipo ataque" contra produccion (autorizado, sin cuentas reales, fines preventivos). Se encontro: DEBUG=False confirmado en el VPS (bien), pero CERO rate limiting en todo el sistema. Se agrego `django-ratelimit` (cache LocMemCache ya configurado, gunicorn corre 1 solo worker asi que no hace falta Redis):
- `/login/` y API login: 15/min por IP.
- `/password_recover`: 5/min por IP (tambien dispara envio de correo).
- Envio de cuestionarios NOM-035 (`SaveAnswers`): 60/min por IP.
Verificado end-to-end contra el server real (bajo el limite responde igual, al exceder responde 403). Commit `6dc812e7`.

### 3. Ajuste de precios NOM-035 PyME/Empresarial -- DESPLEGADO
Socios pidieron subir precios: PyME $1,190 -> $1,790 MXN, Empresarial $2,400 -> $2,800 MXN (ambos anuales, RECURRENTES -- Jorge confirmo explicitamente que se queda como suscripcion auto-renovable, no pago unico, aunque el codigo alguna vez se penso para pago unico). Los precios de Stripe son inmutables una vez usados en una transaccion (aunque fuera de prueba/API, sin cargo real) -- Jorge creo 2 `Price` nuevos en el mismo Producto de Stripe (sin tocar los viejos), se actualizaron `price_id` y `precio` en `stripe_plans.py`. De paso se corrigio el landing publico que decia "Pago unico" (contradecia la realidad, riesgo de publicidad enganosa por auto-renovacion no declarada) -> ahora dice "Renovacion automatica anual - cancela cuando quieras". Tambien se corrigio "Centros de trabajo ilimitados" -> "Hasta 200 centros de trabajo" en las 7 tarjetas de plan que lo decian, consistente con el limite anti-abuso del punto 2. Commit `62a8ea43`.

### 4. Separar "Finalizar aplicacion" de "Nueva evaluacion" -- DESPLEGADO (bug de UX real, no solo cosmetico)
Jorge reporto confusion real probando con su cuenta real: al finalizar, `EndEvaluation` incrementaba `workplace.evaluation` en el mismo paso, asi que cualquier navegacion normal posterior (dashboard, lista de centros) caia por defecto en la evaluacion nueva VACIA sin explicacion -- parecia que los datos recien capturados habian desaparecido. Esto era ademas una decision de diseno documentada del 27 jul (ver seccion de esa fecha arriba), no un accidente -- Jorge decidio revertirla tras ver el efecto real.
- `EndEvaluation` ya NO incrementa `evaluation`, solo registra `EvaluationHistory`. El usuario se queda viendo la evaluacion que acaba de cerrar por defecto.
- Nuevo endpoint `StartNewEvaluation` (`/api/start_new_evaluation/`): solo con clic explicito en boton "Nueva evaluacion" (visible solo cuando la evaluacion actual y vigente ya esta finalizada) se crea el ciclo siguiente.
- Se encontro y cerro un hueco de integridad de datos: como cerrar y avanzar ya no son el mismo paso, un empleado podria responder una encuesta DESPUES de finalizar pero ANTES del clic en "Nueva evaluacion", escribiendo sobre una evaluacion que ya se dio por cerrada. `SaveAnswers` ahora bloquea con 403 cualquier intento de responder una evaluacion ya finalizada (chequeo movido a un punto que cubre ambas rutas del endpoint, valido o invalido el serializer).
- De paso se corrigio la alerta de "cupo de empleados lleno" en Detalle del centro: ya no manda a "seccion de compras" (obsoleto desde el punto 1), ahora invita a crear un centro nuevo (gratis).
- Centros que ya habian auto-avanzado bajo el comportamiento viejo se dejan como estan, sin backfill (cuentas de prueba, se limpiaran aparte).
Verificado con flujo completo real (crear centro, empleado, responder encuesta, finalizar, confirmar NO avance automatico, confirmar bloqueo de envio tardio, confirmar avance solo con clic explicito, confirmar los 4 estados de boton). Commit `8fcd94e3`.

### 5. Dashboard: limpieza visual -- DESPLEGADO
- Tarjeta "Acciones pendientes" ahora se oculta por completo cuando esta vacia (antes mostraba un estado vacio que "solo hacia ruido" segun Jorge). Commit `4dffca70`.
- Tarjeta NOM-035 unificada: los 5 KPIs superiores (centros, empleados, evaluaciones, cumplimiento, riesgo predominante) son datos de NOM-035 pero vivian sueltos arriba de la tarjeta "Centros que necesitan atencion" -- se movieron DENTRO de la misma tarjeta, header pasa a decir solo "NOM-035", "Centros que necesitan atencion" queda como subtitulo interno. Reforzada tambien la distincion visual entre secciones (NOM-035/Psicometria/Acciones/Clima) con borde izquierdo de color solido -- el tono pastel de fondo por si solo hacia que NOM-035 y Psicometria se vieran casi iguales. Commit `02adf986`.
- Badge de plan ("Sin plan activo"/"[Plan] - activo") se perdia en el header de pagina (colores pastel). Se movio al topbar junto al avatar/correo (visible siempre, sin depender de scroll) y se reforzo con colores solidos + pulso sutil en el estado "sin plan". Commit `bc7f1883`.

### 6. SEO -- DESPLEGADO en las 5 paginas de landing
- `rel=canonical` + Twitter Card en las 5 paginas (faltaban en todas).
- JSON-LD: Organization en las 5, + FAQPage en el home (usa el acordeon de FAQ ya existente, 10 preguntas -- habilita rich snippets en Google).
- Meta descriptions reforzadas con keywords objetivo por pagina (home, NOM-035, psicometria, clima laboral), sin relleno forzado.
- Nueva seccion "Lo que arriesgas si no cumples la NOM-035" en el home y en /nom035/, con cifra de multas VERIFICADA por busqueda web antes de publicar (250 a 5,000 UMA, $29,327 a $586,550 MXN segun UMA 2026, Art. 994 fraccion VII LFT) -- aprobada por Jorge explicitamente antes de subirla.
Commits `5ab91553` (home + nom035) y `778a7799` (psicometria + clima).

### Nota de flujo -- entorno local para verificar cada cambio
Toda la sesion se verifico contra Docker local (`docker compose up -d --build web` tras copiar cada archivo tocado con `docker cp`, y esperar a que aparezca "Booting worker" en los logs antes de probar) usando el Django test client (`Client(SERVER_NAME='localhost')` + `force_login`) en vez del navegador -- el Browser pane de Claude no puede tomar screenshot en este entorno (pane no desplegado del lado del usuario), asi que la verificacion fue 100% programatica contra HTML real servido por el server real, no visual. Cuenta de prueba local: `demo_jorge` (password `DemoPass2026!`), datos de prueba creados y limpiados en cada verificacion.

### 7. Plan de contingencia / runbook de incidentes -- NUEVO, en el repo
De cara a las pruebas de carga/ataque con ingenieros externos (autorizadas por Jorge, sin cuentas reales, fines preventivos), se escribio `RUNBOOK_incidentes.md` (raiz del repo): que hacer si el sitio se cae o se comporta mal, 5 escenarios de menor a mayor gravedad -- freno de emergencia (`docker compose stop web`), sitio caido con VPS vivo (diagnostico y reinicio), base de datos danada (restaurar desde backup diario o uno manual antes de las pruebas), droplet colgado sin SSH (consola web de DigitalOcean), y perdida total del droplet (reconstruccion completa desde los backups de Drive). Se documentaron tambien 2 vacios conocidos, no bloqueantes para esta semana: no hay snapshots automaticos del droplet completo (solo datos/config via el backup diario), y no hay firewall de red delante del servidor (solo el rate limiting a nivel de aplicacion agregado en el punto 2b de arriba).

## PENDIENTE INMEDIATO al cerrar esta sesion
1. Confirmar que el ultimo deploy (`git pull && docker compose up -d --build web`) ya corrio en el VPS con el commit `fc66a170` (badge de plan) -- Jorge lo estaba corriendo justo al cerrar esta sesion.
2. Ver memoria `project_revisar_claims_ilimitado_vs_limites_antiabuso`: pendiente revisar el resto del copy de marketing (fuera de landing.html, ya corregido) contra los limites tecnicos reales.
3. Deuda tecnica de migraciones fantasma (EvidenciaFaseC.archivo, PsychoInstrument.tipo choices) sigue sin decision, ver seccion de arriba.
4. `RUNBOOK_incidentes.md` es nuevo y no ha sido probado en un incidente real -- si se usa alguna vez, vale la pena volver y corregir cualquier comando que no haya funcionado tal cual.
