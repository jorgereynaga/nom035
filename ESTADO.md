...
- Templates pdf/politica_prevencion.html + politica_prevencion_form.html (estilo NormaIA real: .form-group/.form-control/.section-card) ✅
...
- Link visible a Politica de Prevencion en evidence.html (tarjeta con href dinamico via v-model workplace de Vue) ✅
- Empty-state de evidence.html corregido: distingue "sin workplace" de "sin resultados" ✅
- Vista GenerarInformeResultadosView (surveys/views.py) — Informe de Resultados NOM-035 numeral 7.7 ✅
- URL generar_informe_resultados/<int:workplace_id>/ ✅
- Template pdf/informe_resultados.html (datos centro, objetivo, actividades, metodo, resultados por dimension con nivel predominante, conclusiones, recomendaciones, responsable) ✅
- Informe de Resultados reutiliza get_chart_data() existente via RequestFactory interno — NO se duplico el calculo de niveles de riesgo por dimension, se reuso la misma fuente de verdad que ya usa el dashboard ✅
- PROBADO EN PRODUCCION 5 Jul: /generar_politica/13/ y /generar_informe_resultados/13/ funcionan correctamente (confirmado por Jorge) ✅
...
- Portafolio Evidencias - Politica de Prevencion: FUNCIONA COMPLETO, probado en produccion ✅
- Portafolio Evidencias - Informe de Resultados: FUNCIONA COMPLETO, probado en produccion 5 Jul ✅

## Pendientes
1. Agregar tarjeta visible del Informe de Resultados en evidence.html (igual que se hizo con la Politica) — actualmente solo accesible por URL directa /generar_informe_resultados/<workplace_id>/
2. Portafolio de evidencias — Fase A restante: CuestionariosAplicadosPDF (listado convocados, % participacion, fechas de aplicacion)
...
4. Portafolio de evidencias — Fase C (despues de A completa): espacios de carga manual para canalizaciones Guia I, examenes medicos, medidas de control, evidencia de difusion
5. Pruebas finales: navegacion entre evaluaciones multiples
6. Selector de "Centro de trabajo" en evidence.html se ve poco estilizado — deuda de diseño PREEXISTENTE confirmada por Jorge (sistema viejo, se esta modernizando), NO es bug de esta sesion. Mejora futura, no urgente
7. Reporte de lentitud de plataforma del 4 Jul — no diagnosticado ni reproducido de nuevo, posiblemente no relacionado a cambios de esta sesion
...
- get_chart_data(request) en views.py: funcion grande (~500 lineas) que calcula niveles de riesgo por dominio/categoria/dimension para TODOS los empleados de un workplace, usando las tablas oficiales de rangos de la Guia II y III de la norma (confirmado que coinciden exactamente con el PDF oficial). Devuelve JsonResponse con claves: status, count, data, total_level, total_cat, data2, total_dim, data5, employees, cat, dimensions, domains. total_dim es una lista de {"value":[indice_dimension, nivel_0a4, cantidad_empleados], "itemStyle":{...}} — para obtener "nivel predominante por dimension" hay que agrupar por indice y tomar el nivel con mayor conteo (moda). dimensions es la lista de nombres en el mismo orden que los indices, con \n incrustado para saltos de linea en graficos (usar chr(10) para reemplazar en vez de '\n' literal si se esta dentro de un string ya anidado)
- Para reusar get_chart_data() sin request HTTP real: usar django.test.RequestFactory, construir fake_request con factory.get() pasando workplace_id/evaluation como querystring, asignar fake_request.user = request.user, y llamar get_chart_data(fake_request) directo. Evita duplicar/reimplementar el calculo de niveles de riesgo
...
- Editor: Sublime Text — siempre editar Python con python -c por indices de linea (readlines()+insert()/lines[n]=...). Al insertar MULTIPLES lineas por indice en una sola pasada, insertar de indice mas alto a mas bajo para no desfasar posiciones. Para bloques largos usar content.replace() con verificacion previa de content.count(marcador)==1 y abortar si no es exactamente 1. SIEMPRE verificar con sed/cat despues de escribir
...
- Portafolio Evidencias Fase A usa tablas oficiales de la norma para niveles de riesgo (deterministico, sin IA generativa libre en conclusiones/recomendaciones — el texto de conclusiones/recomendaciones en informe_resultados.html es fijo/generico, no personalizado por IA)
